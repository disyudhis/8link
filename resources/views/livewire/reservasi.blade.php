<div class="max-w-md mx-auto bg-gradient-to-b from-gray-50 to-white min-h-screen flex flex-col">
    ...
    @if ($booking->assigned_worker_id && in_array($booking->status, ['in_progress', 'completed']))
        <div class="px-5 mb-6">
            <div class="bg-white rounded-2xl shadow-lg p-5 border border-gray-100">
                <h3 class="text-lg font-bold mb-4 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-primary" viewBox="0 0 20 20"
                ...
                <div class="flex items-center space-x-4 mb-4">
                    <div class="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center overflow-hidden">
                        @if ($booking->assignedWorker->photo)
                            <img src="{{ asset('storage/' . $booking->assignedWorker->photo) }}"
                                alt="{{ $booking->assignedWorker->name }}" class="w-full h-full object-cover">
                        @else
                    ...
                    <div class="flex-1">
                        <h4 class="text-lg font-semibold text-gray-800">{{ $booking->assignedWorker->name }}</h4>
                        <p class="text-sm text-gray-600">
                            {{ $booking->assignedWorker->specialization ?? 'Teknisi Cat Kendaraan' }}</p>
                        ...
                        @if ($booking->assignedWorker->phone)
                            <div class="flex items-center mt-1">
                                ...
                                <span class="text-sm text-gray-600">{{ $booking->assignedWorker->phone }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="border-t pt-4 space-y-2">
                    @if ($booking->assigned_at)
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-gray-800">{{ $booking->assigned_at->format('d M Y, H:i') }}</span>
                        </div>
                    @endif

                    @if ($booking->started_at)
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">Mulai dikerjakan:</span>
                            <span class="text-gray-800">{{ $booking->started_at->format('d M Y, H:i') }}</span>
                        </div>
                    @endif
                    ...
                    @if ($booking->completed_at)
                        <div class="flex justify-between text-sm">
                            ...
                            <span
                                class="text-green-600 font-medium">{{ $booking->completed_at->format('d M Y, H:i') }}</span>
                        </div>
                    @endif

                    @if ($booking->status === 'in_progress' && $booking->estimated_completion)
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">Estimasi selesai:</span>
                            <span
                                class="text-blue-600 font-medium">{{ $booking->estimated_completion->format('d M Y, H:i') }}</span>
                        </div>
                    @endif
                </div>
                ...
                @if ($booking->assignedWorker->phone)
                    <div class="mt-4">
                        <a href="https://wa.me/{{ $booking->assignedWorker->phone }}?text={{ urlencode('Halo ' . $booking->assignedWorker->name . ', saya ' . $booking->customer->name . ' pemilik ' . $booking->car_name . ' plat ' . $booking->license_plate . '. Saya ingin menanyakan progres pengerjaan mobil saya.') }}"
                            target="_blank"
                            class="w-full bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg flex items-center justify-center transition duration-300 text-sm">
                            ...
                            Chat Langsung dengan {{ $booking->assignedWorker->name }}
                        </a>
                    </div>
                @endif
            </div>
