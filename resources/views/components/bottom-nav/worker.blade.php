<div x-data="{ showLogoutConfirm: false }" class="flex justify-around items-center h-16 px-3">
    ...
    <a href="{{ route('worker.dashboard') }}"
        class="flex flex-col items-center justify-center w-1/3 h-full {{ request()->routeIs('worker.dashboard') ? 'text-primary' : 'text-gray-600' }}">
    ...
    <a href="{{ route('worker.tasks') }}"
        class="flex flex-col items-center justify-center w-1/3 h-full {{ request()->routeIs('worker.tasks*') ? 'text-primary' : 'text-gray-600' }}">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
